...
def load_omegas(run, timestep=-1, om_avg=True, delta_t_avg=None, t_val=None, check_convergence=True):
...
def plot_comparison_flux_tube_geometry(run, plot_phi=False, zed_times_nfield_periods=False, load_from_nc=True, normalise_bmag=False, colors=None, fig=None, axs=None, norm_gradpar=False):
    ...
    for i, dataObj in enumerate(run.list_dataObj):
        if colors is None:
            ...
            color=colors[i]
        dataObj.plot_flux_tube_geometry(axs=axs, label=run.list_labels[i], plot_phi=plot_phi, zed_times_nfield_periods=zed_times_nfield_periods, load_from_nc=load_from_nc, normalise_bmag=normalise_bmag, color=color, ls=run.list_ls[i], norm_gradpar=norm_gradpar)
...
def plot_phi_vs_zed(run, ax=None, zed_times_nfield_periods=False):
    ...
    for i, dataObj in enumerate(run.list_dataObj):
        dataObj.plot_phi_vs_zed(ax=ax, label=run.list_labels[i], zed_times_nfield_periods=zed_times_nfield_periods)
...
class RunCollection:
    ...
    def __init__(self, filenames_base, labels=None, codes=None, ls=None):
        ...
        for i_file, filename_base in enumerate(self.filenames_base):
            ...
            try:
                dataObj = StellaRun(filename_base, code=code)
    ...
    def load_omegas(self, timestep=-1, om_avg=True, delta_t_avg=None, t_val=None, check_convergence=True):
        return load_omegas(self, timestep=timestep, om_avg=om_avg, delta_t_avg=delta_t_avg, t_val=t_val, check_convergence=check_convergence)
    ...
    def plot_omega_ky(self, fig=None, axs=None, label=None, ls=None, color=None, markersize=10, marker='o', gamma_min=-np.inf, delta_t_avg=None, t_val=None, kx_idx=0, check_convergence=True, rescale_vT=1, rescale_omega=1):
        return scan_omega_scan.plot_omega_ky(self, fig=fig, axs=axs, label=label, ls=ls, color=color, markersize=markersize, marker=marker, gamma_min=gamma_min, delta_t_avg=delta_t_avg, t_val=t_val, kx_idx=kx_idx, check_convergence=check_convergence, rescale_vT=rescale_vT, rescale_omega=rescale_omega)
    ...
    def plot_omega_kx(self, axs=None, label=None, ls=None, color=None, marker='o', gamma_min=-np.inf, delta_t_avg=None, t_val=None, ky_idx=0):
        return scan_omega_scan.plot_omega_kx(self, axs=axs, label=label, ls=ls, color=color, marker=marker, gamma_min=gamma_min, delta_t_avg=delta_t_avg, t_val=t_val, ky_idx=ky_idx)

    def plot_contour_gamma_kx_ky(self, ax=None, delta_t_avg=None, t_val=None):
        return scan_omega_scan.plot_contour_gamma_kx_ky(self, ax=ax, delta_t_avg=delta_t_avg, t_val=t_val)
