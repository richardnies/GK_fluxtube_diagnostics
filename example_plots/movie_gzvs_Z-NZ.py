...
img_dir = "fig_" + dirname_string + "_gzvs_Z-NZ"
...
for i_time_idx, time_idx_val in enumerate(time_idx_vals):
    ...
    try:
        ax = axs[0]
        _, _, im = diagObj.plot_contour_gzvs(time_idx=time_idx_val, logarithmic=True, vmin="symm", nozonal=True, zonal=False, fig=fig, ax=ax)
        plt.colorbar(im, ax=ax)
        ax.set_title(r"$\int\mathrm{d}^2 r_\perp\int\mathrm{d}v_\perp \; g_\mathrm{NZ}^2$")

        ax = axs[1]
        _, _, im = diagObj.plot_contour_gzvs(time_idx=time_idx_val, logarithmic=True, vmin="symm", nozonal=False, zonal=True, fig=fig, ax=ax)
        plt.colorbar(im, ax=ax)
...
os.system('ffmpeg -r 30 -i ' + img_dir + '/fig_t-%03d.png -c:v libx264 -vf fps=30 -pix_fmt yuv420p ' + img_dir + '/video_gzvs_t_'+dirname_string+'.mp4')
