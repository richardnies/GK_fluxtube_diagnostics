"""Movie of g_NZ(vpa, mu) and g_Z(vpa, mu) side by side, vs time, for one run.

Usage:
    python movie_gvmus_Z-NZ.py <config.py>
...
def frame_fn(i, time_idx_val):
    ...
    ax = axs[0]
    _, _, im = run.plot_contour_gvmu_vpa(time_idx=time_idx_val, logarithmic=True, vmin="symm", nozonal=True, zonal=False, fig=fig, ax=ax)
    plt.colorbar(im, ax=ax)
    ax.set_title(r"$V^{-1}\int\mathrm{d}^3 r \;g_\mathrm{NZ}^2$")

    ax = axs[1]
    _, _, im = run.plot_contour_gvmu_vpa(time_idx=time_idx_val, logarithmic=True, vmin="symm", nozonal=False, zonal=True, fig=fig, ax=ax)
    plt.colorbar(im, ax=ax)
...
img_dir = getattr(config, "img_dir", None) or "fig_" + dirname_string + "_gvmus_Z-NZ"

render_movie(
    img_dir, time_idx_vals, frame_fn,
    fps=getattr(config, "fps", 30),
    rerun_all=getattr(config, "rerun_all", True),
    video_name="video_gvmus_t_" + dirname_string + ".mp4",
)
